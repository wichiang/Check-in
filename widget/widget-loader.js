(function () {
  ...
  var loaderVersion = '28.6.19.19.362';
  var scriptVersion = (scriptUrl.searchParams.get('v') || '').trim();
  var assetVersion = (scriptVersion || loaderVersion || '').trim();
  ...
  function buildLoaderDiagnostics() {
    ...
    if (!hasVersionQuery) {
      issues.push('widget-loader.js 的 src 沒有帶 ?v= 版本參數。');
      suggestions.push('請將 src 改為 widget-loader.js?v=' + loaderVersion + '。Loader 會自動把同一版本套用到 CSS、bundle 與 vendor 資源。');
    }
    ...
    if (scriptVersion && scriptVersion !== loaderVersion) {
      ...
      suggestions.push('請確認 IIS 實際部署目錄、外部嵌入頁、Proxy/CDN 或 CMS 快取是否仍保留舊版 loader 網址。');
    }

    if (rawScriptSrc && rawScriptSrc.indexOf('?') < 0) {
      suggestions.push('若重新整理後仍載入舊版，通常是外部 HTML、Layout、CMS 區塊或 IIS/Proxy 快取仍保留沒有 ?v= 的舊嵌入碼。');
    }
    ...
    return {
      ...
      versionSource: scriptVersion ? 'script-src-v' : 'loader-internal-fallback',
  ...
  function publishLoaderDiagnostics() {
    ...
    if (diagnostics.issues.length > 0 && window.console && typeof window.console.warn === 'function') {
      ...
      console.info('AI Assistant Widget 版本診斷：單一 src ?v= 版本治理正常。', diagnostics);
    }
  ...
  function readDatasetInteger(key, fallback) {
    if (!Object.prototype.hasOwnProperty.call(currentScript.dataset, key)) return fallback;
    var text = String(currentScript.dataset[key] == null ? '' : currentScript.dataset[key]).trim();
    ...
    .then(function () {
      if (window.AIAssistantWidget) {
        ...
        window.AIAssistantWidget.create({
          ...
          // v28.6.19.19.346：前端瀏覽器暫存設定，只影響 localStorage 跨頁還原；未設定 data-* 時可由後台站台設定接手。
          conversationPersistenceEnabled: parseDatasetBoolean('conversationPersistenceEnabled', true),
          hasCustomConversationPersistenceEnabled: hasDatasetBoolean('conversationPersistenceEnabled'),
          conversationPersistenceHours: readDatasetInteger('conversationPersistenceHours', 24),
          hasCustomConversationPersistenceHours: hasDatasetText('conversationPersistenceHours'),
          conversationPersistenceMaxMessages: readDatasetInteger('conversationPersistenceMaxMessages', 40),
          hasCustomConversationPersistenceMaxMessages: hasDatasetText('conversationPersistenceMaxMessages'),
          // v28.6.19.19.362：多對話預設停用；明確啟用後才顯示對話清單與新增／切換／刪除功能。
          multiConversationEnabled: parseDatasetBoolean('multiConversationEnabled', false),
          hasCustomMultiConversationEnabled: hasDatasetBoolean('multiConversationEnabled'),
          conversationHistoryMaxItems: readDatasetInteger('conversationHistoryMaxItems', 5),
