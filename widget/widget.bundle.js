(function (global) {
  ...
  const DEFAULTS = {
    ...
    // v28.6.19.19.304：前端瀏覽器暫存設定可由後台 SITE_BRANDING 或 data-* 控制。
    // 只影響 localStorage 的跨頁還原，不等同後端 DB 對話紀錄保存期限。
    conversationPersistenceEnabled: true,
    conversationPersistenceHours: 24,
    conversationPersistenceMaxMessages: 40,
    // v28.6.19.19.362：預設維持單一對話；只有明確啟用才顯示多對話工作區。
    multiConversationEnabled: false,
    conversationHistoryMaxItems: 5,
    sessionCloseEndpoint: '/api/widget/session/close'
  };

  // 跨頁對話保留設定：同一瀏覽器、同一站台與同一助理在換頁後可還原最近對話。
  // v28.6.19.19.304：TTL 與訊息數可由後台 SITE_BRANDING 或 data-conversation-persistence-* 控制；JSON 大小仍保留內部安全上限。
  ...
  // v28.6.19.19.362：多對話工作區使用 IndexedDB 非同步保存，避免多個完整對話阻塞主執行緒。
  // 瀏覽器不支援或拒絕 IndexedDB 時，會退回 localStorage；功能仍可使用，但容量較小。
  const MULTI_CONVERSATION_DB_NAME = 'ai-assistant-widget-conversations-v1';
  const MULTI_CONVERSATION_DB_STORE = 'workspaces';

  function openMultiConversationDatabase() {
    return new Promise((resolve, reject) => {
      if (!global.indexedDB) { reject(new Error('IndexedDB unavailable')); return; }
      const request = global.indexedDB.open(MULTI_CONVERSATION_DB_NAME, 1);
      request.onupgradeneeded = () => {
        const database = request.result;
        if (!database.objectStoreNames.contains(MULTI_CONVERSATION_DB_STORE)) {
          database.createObjectStore(MULTI_CONVERSATION_DB_STORE, { keyPath: 'key' });
        }
      };
  ...
  async function readMultiConversationWorkspace(key) {
    try {
      const database = await openMultiConversationDatabase();
      return await new Promise((resolve, reject) => {
        const tx = database.transaction(MULTI_CONVERSATION_DB_STORE, 'readonly');
        const request = tx.objectStore(MULTI_CONVERSATION_DB_STORE).get(key);
      ...
      const raw = safeStorageGetItem(key + ':workspace');
      if (!raw) return null;
      try { return JSON.parse(raw); } catch (_) { return null; }
    }
  }

  async function writeMultiConversationWorkspace(workspace) {
    if (!workspace || !workspace.key) return false;
    try {
      const database = await openMultiConversationDatabase();
      await new Promise((resolve, reject) => {
        const tx = database.transaction(MULTI_CONVERSATION_DB_STORE, 'readwrite');
        tx.objectStore(MULTI_CONVERSATION_DB_STORE).put(workspace);
        tx.oncomplete = resolve;
        tx.onerror = () => reject(tx.error || new Error('IndexedDB write failed'));
        tx.onabort = () => reject(tx.error || new Error('IndexedDB write aborted'));
      });
      database.close();
      safeStorageRemoveItem(workspace.key + ':workspace');
      return true;
    } catch (_) {
      try {
        const json = JSON.stringify(workspace);
        if (json.length > CONVERSATION_PERSISTENCE_MAX_JSON_CHARS * 5) return false;
        return safeStorageSetItem(workspace.key + ':workspace', json);
      } catch (_) {
        return false;
      }
    }
  ...
  async function deleteMultiConversationWorkspace(key) {
    safeStorageRemoveItem(key + ':workspace');
    try {
      const database = await openMultiConversationDatabase();
      await new Promise((resolve, reject) => {
        const tx = database.transaction(MULTI_CONVERSATION_DB_STORE, 'readwrite');
        tx.objectStore(MULTI_CONVERSATION_DB_STORE).delete(key);
        tx.oncomplete = resolve;
  ...
  function normalizeConversationPersistenceHours(value, fallback) {
    // 前端 localStorage 不建議無限制保存；正式建議 24 小時，最高 720 小時（30 天）。
    return normalizeIntegerRange(value, fallback, 1, 720);
  }

  function normalizeConversationPersistenceMaxMessages(value, fallback) {
    // 避免瀏覽器儲存膨脹與跨頁還原過重，每個對話最高保存最近 100 則。
    return normalizeIntegerRange(value, fallback, 1, 100);
  }

  function normalizeConversationHistoryMaxItems(value, fallback) {
    // 多對話模式最多 20 個，避免共用瀏覽器累積過多前端對話內容。
    return normalizeIntegerRange(value, fallback, 1, 20);
  }
  ...
  async function apiFetch(base, path, options) {
    ...
    if (!response.ok) {
      ...
      const error = new Error(detail || 'API 呼叫失敗');
      error.status = response.status;
      error.problem = data && typeof data === 'object' ? data : null;
      throw error;
    }
  ...
  function isRecoverableWidgetSessionError(error) {
    const status = Number(error && error.status || 0);
    const message = String(error && error.message || '');
    if (status === 404 && /工作階段|Session/i.test(message)) return true;
    return status === 400 && /工作階段.*(非開啟|已關閉|不存在|找不到)|Session.*(closed|not found)/i.test(message);
  }
  ...
  function buildBrandShort(config) {
    // 品牌徽章文字只接受明確的 brandShort。
    // 不再由 brandName、站台名稱或標題自動推導，避免未設定時出現來源不明的單一字母。
    return String(config.brandShort || '').trim().slice(0, 5);
  }


  function shouldShowBrandPill(config) {
    // 只有明確設定品牌短名時才顯示徽章。brandName 只保留為完整品牌資訊，不再自動產生短名。
    return String(config.brandShort || '').trim().length > 0;
  }
  ...
  const INLINE_ICON_PATHS = {
    ...
    trash: 'M4 7h16M9 7V4h6v3M8 11v6M12 11v6M16 11v6M6 7l1 14h10l1-14',
    edit: 'M4 20h4l11-11a2.8 2.8 0 0 0-4-4L4 16v4M13.5 6.5l4 4',
  ...
  class AIAssistantWidget {
    constructor(config) {
      ...
      this.config.hasCustomConversationPersistenceEnabled = hasFlag('hasCustomConversationPersistenceEnabled') ? !!(config || {}).hasCustomConversationPersistenceEnabled : (Object.prototype.hasOwnProperty.call(config || {}, 'conversationPersistenceEnabled') && typeof (config || {}).conversationPersistenceEnabled === 'boolean');
      this.config.hasCustomConversationPersistenceHours = hasFlag('hasCustomConversationPersistenceHours') ? !!(config || {}).hasCustomConversationPersistenceHours : hasTextOverride('conversationPersistenceHours');
      this.config.hasCustomConversationPersistenceMaxMessages = hasFlag('hasCustomConversationPersistenceMaxMessages') ? !!(config || {}).hasCustomConversationPersistenceMaxMessages : hasTextOverride('conversationPersistenceMaxMessages');
      this.config.hasCustomMultiConversationEnabled = hasFlag('hasCustomMultiConversationEnabled') ? !!(config || {}).hasCustomMultiConversationEnabled : (Object.prototype.hasOwnProperty.call(config || {}, 'multiConversationEnabled') && typeof (config || {}).multiConversationEnabled === 'boolean');
      this.config.hasCustomConversationHistoryMaxItems = hasFlag('hasCustomConversationHistoryMaxItems') ? !!(config || {}).hasCustomConversationHistoryMaxItems : hasTextOverride('conversationHistoryMaxItems');
      ...
      this.config.conversationPersistenceEnabled = parseOptionalBoolean(this.config.conversationPersistenceEnabled, DEFAULTS.conversationPersistenceEnabled) !== false;
      this.config.conversationPersistenceHours = normalizeConversationPersistenceHours(this.config.conversationPersistenceHours, DEFAULTS.conversationPersistenceHours);
      this.config.conversationPersistenceMaxMessages = normalizeConversationPersistenceMaxMessages(this.config.conversationPersistenceMaxMessages, DEFAULTS.conversationPersistenceMaxMessages);
      this.config.multiConversationEnabled = parseOptionalBoolean(this.config.multiConversationEnabled, DEFAULTS.multiConversationEnabled) === true;
      this.config.conversationHistoryMaxItems = normalizeConversationHistoryMaxItems(this.config.conversationHistoryMaxItems, DEFAULTS.conversationHistoryMaxItems);
      // 對話識別與回饋綁定狀態。
      this.sessionId = null;
      ...
      this.statusEl = null;
      this.sendBtnEl = null;
      this.headerResetBtnEl = null;
      this.inputResetBtnEl = null;
      // v28.6.19.19.301：引導式 FAQ 進行中時，在輸入區上方提供常駐離開入口，避免使用者必須往上滑回最後一個選擇區塊。
      this.guidedFaqStatusBarEl = null;
      this.guidedFaqStatusTextEl = null;
      this.guidedFaqStatusExitBtnEl = null;
      this.isBusy = false;
      ...
      this.resetConfirmTitleEl = null;
      this.resetConfirmDescEl = null;
      this.resetConfirmOkBtnEl = null;
      ...
      // v28.6.19.19.301：記錄引導式 FAQ 目前已選擇的路徑，供回答卡片以輕量歷程呈現。
      this.pendingGuidedFaqSelectedText = '';
      // v28.6.19.19.301：點選最後一層選項後允許重新選擇不同答案。
      // 記錄選項區塊當下的歷程基準，避免重新點選同一層其他答案時把舊答案選項串進歷程。
      this.pendingGuidedFaqTrailBase = null;
      this.activeGuidedFaqTrail = [];
      ...
      // v28.6.19.19.362：多對話狀態只在 multiConversationEnabled=true 時使用；主畫面的重新開始與清單內新增對話維持不同責任。
      this.multiConversationWorkspaceKey = '';
      this.multiConversationItems = [];
      this.activeConversationId = '';
      this.multiConversationSaveQueue = Promise.resolve();
      this.conversationDrawerEl = null;
      this.conversationListEl = null;
      this.conversationDrawerBtnEl = null;
      this.conversationDrawerLastFocusedEl = null;
    }
    ...
    getRuntimeSummary() {
      ...
      return {
        ...
        conversationPersistenceEnabled: this.config.conversationPersistenceEnabled !== false,
        conversationPersistenceHours: this.config.conversationPersistenceHours,
        conversationPersistenceMaxMessages: this.config.conversationPersistenceMaxMessages,
        multiConversationEnabled: this.config.multiConversationEnabled === true,
        conversationHistoryMaxItems: this.config.conversationHistoryMaxItems,
        activeConversationId: this.activeConversationId || '',
        conversationCount: Array.isArray(this.multiConversationItems) ? this.multiConversationItems.length : 0,
    ...
    syncRuntimeSummary(reason) {
      ...
      if (this.rootEl) {
        ...
        this.rootEl.dataset.configConversationPersistenceEnabled = summary.conversationPersistenceEnabled ? 'true' : 'false';
        this.rootEl.dataset.configConversationPersistenceHours = String(summary.conversationPersistenceHours || '');
        this.rootEl.dataset.configConversationPersistenceMaxMessages = String(summary.conversationPersistenceMaxMessages || '');
        this.rootEl.dataset.configMultiConversationEnabled = summary.multiConversationEnabled ? 'true' : 'false';
        this.rootEl.dataset.configConversationHistoryMaxItems = String(summary.conversationHistoryMaxItems || '');
        this.rootEl.dataset.runtimeActiveConversationId = summary.activeConversationId;
        this.rootEl.dataset.runtimeConversationCount = String(summary.conversationCount || 0);
    ...
    async init() {
      ...
      this.render();
      await this.restorePersistentConversation();
      ...
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          if (!this.config.hasCustomSubtitle) {
            const remoteSubtitle = typeof data.widgetSubtitle === 'string'
              ? data.widgetSubtitle
              : typeof data.subtitle === 'string'
                ? data.subtitle
                : '';
            this.config.subtitle = data.hideSubtitle === true
              ? ''
              : (remoteSubtitle.trim().length > 0 ? remoteSubtitle.trim() : this.config.subtitle);
          }
          ...
          if (!this.config.hasCustomConversationPersistenceEnabled && typeof data.conversationPersistenceEnabled === 'boolean') this.config.conversationPersistenceEnabled = data.conversationPersistenceEnabled;
          if (!this.config.hasCustomConversationPersistenceHours && data.conversationPersistenceHours) this.config.conversationPersistenceHours = normalizeConversationPersistenceHours(data.conversationPersistenceHours, this.config.conversationPersistenceHours);
          if (!this.config.hasCustomConversationPersistenceMaxMessages && data.conversationPersistenceMaxMessages) this.config.conversationPersistenceMaxMessages = normalizeConversationPersistenceMaxMessages(data.conversationPersistenceMaxMessages, this.config.conversationPersistenceMaxMessages);
          if (!this.config.hasCustomMultiConversationEnabled && typeof data.multiConversationEnabled === 'boolean') this.config.multiConversationEnabled = data.multiConversationEnabled;
          if (!this.config.hasCustomConversationHistoryMaxItems && data.conversationHistoryMaxItems) this.config.conversationHistoryMaxItems = normalizeConversationHistoryMaxItems(data.conversationHistoryMaxItems, this.config.conversationHistoryMaxItems);
          if (!this.config.hasCustomBrandShort && typeof data.brandShort === 'string') this.config.brandShort = data.brandShort.trim().slice(0, 5);
          if (!this.config.hasCustomBrandName && typeof data.brandName === 'string') this.config.brandName = data.brandName.trim();
    ...
    render() {
      ...
      root.classList.add(this.config.multiConversationEnabled ? 'multi-conversation-enabled' : 'single-conversation-mode');
      ...
      if (this.config.subtitle) modalShell.setAttribute('aria-describedby', this.subtitleId);
      ...
      if (this.config.multiConversationEnabled) {
        const conversationDrawerBtn = createEl('button', 'ai-assistant-widget-icon-btn btn btn-sm', '');
        conversationDrawerBtn.type = 'button';
        setSvgButtonIcon(conversationDrawerBtn, INLINE_ICON_PATHS.list, '對話清單');
        conversationDrawerBtn.setAttribute('aria-expanded', 'false');
        conversationDrawerBtn.addEventListener('click', () => this.openConversationDrawer());
        this.conversationDrawerBtnEl = conversationDrawerBtn;
        headerActions.appendChild(conversationDrawerBtn);
      }
      ...
      this.headerResetBtnEl = clearBtn;
      ...
      const guidedFaqStatusBar = createEl('div', 'ai-assistant-widget-guided-status-bar');
      guidedFaqStatusBar.hidden = true;
      guidedFaqStatusBar.setAttribute('aria-hidden', 'true');
      guidedFaqStatusBar.setAttribute('aria-live', 'polite');
      guidedFaqStatusBar.setAttribute('aria-label', '引導式 FAQ 狀態');
      const guidedFaqStatusText = createEl('div', 'ai-assistant-widget-guided-status-text', '正在依選項引導回答');
      const guidedFaqStatusExitBtn = createEl('button', 'ai-assistant-widget-guided-status-exit', '離開引導');
      guidedFaqStatusExitBtn.type = 'button';
      guidedFaqStatusExitBtn.title = '離開目前的引導式 FAQ，回到一般提問模式。';
      guidedFaqStatusExitBtn.setAttribute('aria-label', '離開引導式 FAQ，回到一般提問模式');
      guidedFaqStatusExitBtn.addEventListener('click', () => {
        if (this.isBusy || guidedFaqStatusExitBtn.disabled) return;
        this.exitGuidedFaqFlow();
      });
      guidedFaqStatusBar.appendChild(guidedFaqStatusText);
      guidedFaqStatusBar.appendChild(guidedFaqStatusExitBtn);
      this.guidedFaqStatusBarEl = guidedFaqStatusBar;
      this.guidedFaqStatusTextEl = guidedFaqStatusText;
      this.guidedFaqStatusExitBtnEl = guidedFaqStatusExitBtn;
      footer.appendChild(guidedFaqStatusBar);
      ...
      const resetConfirmInput = createEl('input', 'ai-assistant-widget-reset-confirm-input');
      resetConfirmInput.type = 'text';
      resetConfirmInput.maxLength = 40;
      resetConfirmInput.autocomplete = 'off';
      resetConfirmInput.hidden = true;
      resetConfirmInput.setAttribute('aria-label', '對話名稱');
      ...
      resetConfirmCard.appendChild(resetConfirmInput);
      ...
      resetOkBtn.addEventListener('click', () => {
        if (this.resetConfirmMode === 'rename') {
          const value = String(resetConfirmInput.value || '').replace(/\s+/g, ' ').trim();
          if (!value) {
            this.setStatus('請輸入對話名稱。', { tone: 'warning', duration: 2600 });
            resetConfirmInput.focus();
            return;
          }
          this.closeResetConfirmation(value.slice(0, 40));
          return;
        }
        this.closeResetConfirmation(true);
      });
      this.resetConfirmEl = resetConfirm;
      this.resetConfirmTitleEl = resetConfirmTitle;
      this.resetConfirmDescEl = resetConfirmDesc;
      this.resetConfirmOkBtnEl = resetOkBtn;
      this.resetConfirmInputEl = resetConfirmInput;
      this.resetConfirmMode = 'confirm';

      if (this.config.multiConversationEnabled) {
        panel.appendChild(this.buildConversationDrawer());
      }
    ...
    getMultiConversationWorkspaceKey() {
      if (this.multiConversationWorkspaceKey) return this.multiConversationWorkspaceKey;
      this.multiConversationWorkspaceKey = this.getConversationPersistenceKey().replace(':conversation:', ':conversation-workspace:');
      return this.multiConversationWorkspaceKey;
    }

    isConversationPersistenceEnabled() {
      return this.config && this.config.conversationPersistenceEnabled !== false && Number(this.config.conversationPersistenceHours || 0) > 0;
    }

    isMultiConversationEnabled() {
      return !!(this.config && this.config.multiConversationEnabled === true);
    }

    getConversationPersistenceTtlMs() {
      return normalizeConversationPersistenceHours(this.config && this.config.conversationPersistenceHours, DEFAULTS.conversationPersistenceHours) * 60 * 60 * 1000;
    }

    getConversationPersistenceMaxMessages() {
      return normalizeConversationPersistenceMaxMessages(this.config && this.config.conversationPersistenceMaxMessages, DEFAULTS.conversationPersistenceMaxMessages);
    }

    getConversationHistoryMaxItems() {
      return normalizeConversationHistoryMaxItems(this.config && this.config.conversationHistoryMaxItems, DEFAULTS.conversationHistoryMaxItems);
    }
    ...
    buildConversationSummary(messages) {
      ...
      const latest = [...messages].reverse().find((item) => item && item.text && !isSystemFailureText(item.text));
      if (!latest) return '';
      const text = String(latest.text || '').replace(/[\r\n\t]+/g, ' ').replace(/\s+/g, ' ').trim();
      return text.length > 46 ? text.slice(0, 46) + '…' : text;
    }

    buildCurrentConversationSnapshot() {
      const now = Date.now();
      return {
        schemaVersion: 3,
        savedAt: now,
        expiresAt: now + this.getConversationPersistenceTtlMs(),
        retentionHours: this.config.conversationPersistenceHours,
        maxMessages: this.getConversationPersistenceMaxMessages(),
        ...
        hasConversationStarted: !!this.hasConversationStarted,
        firstScreenVariant: this.firstScreenVariant || (this.hasConversationStarted ? 'conversation' : 'reset'),
        suggestionsExpanded: !!this.suggestionsExpanded,
        activeGuidedFaqOptions: sanitizeGuidedOptionsForPersistence(this.activeGuidedFaqOptions),
        activeGuidedFaqTrail: this.sanitizeGuidedFaqTrail(this.activeGuidedFaqTrail),
        messages: Array.isArray(this.persistedMessages) ? this.persistedMessages.slice(-this.getConversationPersistenceMaxMessages()) : []
      };
    }

    buildEmptyConversationRecord() {
      const now = Date.now();
      return {
        ...
        snapshot: {
          ...
          firstScreenVariant: 'reset',
          suggestionsExpanded: this.config.suggestionsMode !== 'collapsed',
          activeGuidedFaqOptions: [],
          activeGuidedFaqTrail: [],
          messages: []
        }
      };
    }
    ...
    addPersistentMessageRecord(record) {
      ...
      const maxMessages = this.getConversationPersistenceMaxMessages();
      if (this.persistedMessages.length > maxMessages) {
        this.persistedMessages = this.persistedMessages.slice(-maxMessages);
      }
      if (this.isMultiConversationEnabled() && record.role === 'user') {
        const current = this.multiConversationItems.find((item) => item.id === this.activeConversationId);
        if (current && !current.customTitle && (!current.title || current.title === '新對話')) current.title = this.buildConversationTitle(record.text);
      }
    }

    persistConversationState() {
      if (this.isRestoringConversation) return;
      if (this.isMultiConversationEnabled()) {
        this.persistMultiConversationState();
        return;
      }
      this.persistSingleConversationState();
    }

    persistSingleConversationState() {
      if (!this.isConversationPersistenceEnabled()) {
        this.clearPersistentConversation();
        return;
      }
      if (!Array.isArray(this.persistedMessages) || this.persistedMessages.length === 0) return;
      const snapshot = this.buildCurrentConversationSnapshot();
      const saved = safeStorageSetItem(this.getConversationPersistenceKey(), JSON.stringify(snapshot));
      if (!saved) {
        const compactSnapshot = { ...snapshot, messages: this.persistedMessages.slice(-Math.min(12, this.getConversationPersistenceMaxMessages())) };
    ...
    captureCurrentMultiConversation() {
      if (!this.activeConversationId) return null;
      let current = this.multiConversationItems.find((item) => item.id === this.activeConversationId);
      if (!current) {
        current = this.buildEmptyConversationRecord();
        current.id = this.activeConversationId;
        this.multiConversationItems.push(current);
      }
      const now = Date.now();
      const snapshot = this.buildCurrentConversationSnapshot();
      ...
      current.summary = this.buildConversationSummary(snapshot.messages);
      if (!current.customTitle && (!current.title || current.title === '新對話')) {
        const firstUser = snapshot.messages.find((item) => item && item.role === 'user' && item.text);
        current.title = firstUser ? this.buildConversationTitle(firstUser.text) : '新對話';
      }
      return current;
    }

    buildMultiConversationWorkspace() {
      return {
        key: this.getMultiConversationWorkspaceKey(),
        schemaVersion: 1,
        activeConversationId: this.activeConversationId || '',
        updatedAt: Date.now(),
        conversations: this.multiConversationItems.map((item) => cloneForPersistence(item, CONVERSATION_PERSISTENCE_MAX_JSON_CHARS * 2)).filter(Boolean)
      };
    }

    persistMultiConversationState() {
      this.captureCurrentMultiConversation();
      this.renderConversationList();
      this.syncRuntimeSummary('multi-conversation-persist');
      if (!this.isConversationPersistenceEnabled()) return;
      const workspace = this.buildMultiConversationWorkspace();
      this.multiConversationSaveQueue = this.multiConversationSaveQueue
        .catch(() => {})
        .then(() => writeMultiConversationWorkspace(workspace));
    }

    async restorePersistentConversation() {
      if (this.isMultiConversationEnabled()) return await this.restoreMultiConversationWorkspace();

      // 管理者由多對話切回單一對話時，保留最後使用中的對話作為單一對話，不合併其他主題。
      if (!safeStorageGetItem(this.getConversationPersistenceKey())) {
        const workspace = await readMultiConversationWorkspace(this.getMultiConversationWorkspaceKey());
        const active = workspace && Array.isArray(workspace.conversations)
          ? workspace.conversations.find((item) => item.id === workspace.activeConversationId) || workspace.conversations[0]
          : null;
        if (active && active.snapshot && Array.isArray(active.snapshot.messages) && active.snapshot.messages.length) {
          safeStorageSetItem(this.getConversationPersistenceKey(), JSON.stringify(active.snapshot));
          await deleteMultiConversationWorkspace(this.getMultiConversationWorkspaceKey());
        }
      }
      return this.restoreSinglePersistentConversation();
    }

    restoreSinglePersistentConversation() {
      ...
      const now = Date.now();
      if (!snapshot || ![1, 2, 3].includes(Number(snapshot.schemaVersion || 0)) || !Array.isArray(snapshot.messages) || snapshot.messages.length === 0 || Number(snapshot.expiresAt || 0) <= now) {
        this.clearPersistentConversation();
        return false;
      }
      return this.applyConversationSnapshot(snapshot);
    }

    async restoreMultiConversationWorkspace() {
      if (!this.messagesEl) return false;
      let workspace = this.isConversationPersistenceEnabled()
        ? await readMultiConversationWorkspace(this.getMultiConversationWorkspaceKey())
        : null;
      const now = Date.now();
      let conversations = workspace && Array.isArray(workspace.conversations)
        ? workspace.conversations.filter((item) => item && item.id && Number(item.expiresAt || item.snapshot?.expiresAt || 0) > now)
        : [];

      // 第一次啟用多對話時，將既有單一對話轉為第一筆，不清除使用者目前內容。
      if (!conversations.length) {
        const legacyRaw = safeStorageGetItem(this.getConversationPersistenceKey());
        if (legacyRaw) {
          try {
            const legacySnapshot = JSON.parse(legacyRaw);
            if (legacySnapshot && Array.isArray(legacySnapshot.messages) && legacySnapshot.messages.length && Number(legacySnapshot.expiresAt || 0) > now) {
              const firstUser = legacySnapshot.messages.find((item) => item && item.role === 'user' && item.text);
              conversations.push({
                id: this.createConversationId(),
                title: firstUser ? this.buildConversationTitle(firstUser.text) : '既有對話',
                summary: this.buildConversationSummary(legacySnapshot.messages),
                createdAt: Number(legacySnapshot.savedAt || now),
                updatedAt: Number(legacySnapshot.savedAt || now),
                expiresAt: Number(legacySnapshot.expiresAt || now + this.getConversationPersistenceTtlMs()),
                snapshot: { ...legacySnapshot, schemaVersion: 3 }
              });
              safeStorageRemoveItem(this.getConversationPersistenceKey());
            }
          } catch (_) {}
      ...
      if (!conversations.length) conversations.push(this.buildEmptyConversationRecord());

      this.multiConversationItems = conversations.sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
      this.activeConversationId = workspace && this.multiConversationItems.some((item) => item.id === workspace.activeConversationId)
        ? workspace.activeConversationId
        : this.multiConversationItems[0].id;
      await this.enforceMultiConversationLimit();
      const active = this.multiConversationItems.find((item) => item.id === this.activeConversationId) || this.multiConversationItems[0];
      this.activeConversationId = active.id;
      this.applyConversationSnapshot(active.snapshot || this.buildEmptyConversationRecord().snapshot);
      this.renderConversationList();
      this.persistMultiConversationState();
      return !!(active.snapshot && Array.isArray(active.snapshot.messages) && active.snapshot.messages.length);
    }
    ...
    applyConversationSnapshot(snapshot) {
      ...
      try {
        this.messagesEl.innerHTML = '';
        this.emptyStateEl = null;
        this.sessionId = snapshot && snapshot.sessionId ? snapshot.sessionId : null;
        this.lastAssistantMessageId = snapshot && snapshot.lastAssistantMessageId ? snapshot.lastAssistantMessageId : null;
        this.lastUserMessageText = snapshot && snapshot.lastUserMessageText ? snapshot.lastUserMessageText : '';
        const messages = snapshot && Array.isArray(snapshot.messages) ? snapshot.messages : [];
        this.hasConversationStarted = messages.length > 0 || !!(snapshot && snapshot.hasConversationStarted);
        this.firstScreenVariant = this.hasConversationStarted ? 'conversation' : (snapshot && snapshot.firstScreenVariant === 'initial' ? 'initial' : 'reset');
        this.suggestionsExpanded = this.hasConversationStarted ? false : (snapshot ? snapshot.suggestionsExpanded !== false : true);
        this.activeGuidedFaqOptions = [];
        this.activeGuidedFaqTrail = [];
        this.pendingGuidedFaqAction = '';
        this.pendingGuidedFaqSelectedText = '';
        ...
        if (messages.length) {
          messages.forEach((record) => this.restorePersistentMessageRecord(record));
          ...
          this.activeGuidedFaqTrail = this.sanitizeGuidedFaqTrail(snapshot.activeGuidedFaqTrail || this.activeGuidedFaqTrail);
        } else {
          this.renderEmptyState();
        }
        this.persistedMessages = messages.slice(-this.getConversationPersistenceMaxMessages());
        this.syncGuidedFaqStatusBar();
        this.applySuggestionsState();
        this.updateFirstScreenState();
    ...
    async enforceMultiConversationLimit() {
      const maxItems = this.getConversationHistoryMaxItems();
      this.multiConversationItems.sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
      if (this.multiConversationItems.length <= maxItems) return;
      const keep = [];
      const removed = [];
      this.multiConversationItems.forEach((item) => {
        if (keep.length < maxItems || item.id === this.activeConversationId) keep.push(item);
        else removed.push(item);
      });
      while (keep.length > maxItems) {
        const index = keep.findLastIndex ? keep.findLastIndex((item) => item.id !== this.activeConversationId) : (() => {
          for (let i = keep.length - 1; i >= 0; i -= 1) if (keep[i].id !== this.activeConversationId) return i;
          return -1;
        })();
        if (index < 0) break;
        removed.push(keep.splice(index, 1)[0]);
      }
      this.multiConversationItems = keep;
      removed.forEach((item) => this.closeSessionQuietly(item && item.snapshot && item.snapshot.sessionId));
    }

    async closeSessionQuietly(sessionId) {
      if (!sessionId || !this.config.sessionCloseEndpoint) return;
      try {
        await apiFetch(this.config.apiBase, this.config.sessionCloseEndpoint, {
          method: 'POST',
          body: JSON.stringify({ sessionId, visitorId: this.getOrCreateVisitorId() })
        });
      } catch (_) {}
    }

    resetCurrentConversationState(firstScreenVariant = 'reset') {
      this.stopLoadingStages();
      ...
      this.persistedMessages = [];
      this.hasConversationStarted = false;
      this.firstScreenVariant = firstScreenVariant;
    ...
    async createNewConversation(focusInput = false) {
      if (!this.isMultiConversationEnabled()) return;
      if (this.isBusy) {
        this.setStatus('目前正在處理回答，完成後即可新增對話。', { tone: 'info', duration: 3600 });
        return;
      }
      this.captureCurrentMultiConversation();
      const record = this.buildEmptyConversationRecord();
      this.multiConversationItems.unshift(record);
      this.activeConversationId = record.id;
      await this.enforceMultiConversationLimit();
      this.resetCurrentConversationState('reset');
      this.closeConversationDrawer();
      await this.ensureSession();
      this.captureCurrentMultiConversation();
      this.persistMultiConversationState();
      this.setStatus('已建立新對話。', { tone: 'success' });
      if (focusInput && this.textareaEl && !this.textareaEl.disabled) this.textareaEl.focus();
    }

    async switchConversation(conversationId) {
      if (!this.isMultiConversationEnabled() || !conversationId || conversationId === this.activeConversationId) {
        this.closeConversationDrawer();
        return;
      }
      if (this.isBusy) {
        this.setStatus('目前正在處理回答，完成後即可切換對話。', { tone: 'info', duration: 3600 });
        return;
      }
      this.captureCurrentMultiConversation();
      const target = this.multiConversationItems.find((item) => item.id === conversationId);
      if (!target) return;
      target.updatedAt = Date.now();
      this.activeConversationId = target.id;
      this.applyConversationSnapshot(target.snapshot || this.buildEmptyConversationRecord().snapshot);
      this.closeConversationDrawer();
      await this.ensureSession();
      this.captureCurrentMultiConversation();
      this.persistMultiConversationState();
      this.setStatus(`已切換至「${target.title || '新對話'}」。`, { tone: 'success' });
    }

    async renameConversation(conversationId) {
      if (!this.isMultiConversationEnabled() || !conversationId) return;
      if (this.isBusy) {
        this.setStatus('目前正在處理回答，完成後即可重新命名。', { tone: 'info', duration: 3600 });
        return;
      }
      const target = this.multiConversationItems.find((item) => item.id === conversationId);
      if (!target) return;
      const nextTitle = await this.showConversationRenameDialog(target.title || '新對話');
      if (!nextTitle) return;
      ...
      this.persistMultiConversationState();
      this.renderConversationList();
      this.setStatus(`已將對話重新命名為「${nextTitle}」。`, { tone: 'success' });
    }

    async deleteConversation(conversationId) {
      if (!this.isMultiConversationEnabled() || !conversationId) return;
      if (this.isBusy) {
        this.setStatus('目前正在處理回答，完成後即可刪除對話。', { tone: 'info', duration: 3600 });
        return;
      }
      const target = this.multiConversationItems.find((item) => item.id === conversationId);
      if (!target) return;
      const confirmed = await this.showActionConfirmation(
        '刪除此對話？',
        '刪除後，此瀏覽器將不再顯示這段對話，且無法復原。後端紀錄仍依站台保存政策保留。',
        '確認刪除',
        true
      );
      if (!confirmed) return;
      await this.closeSessionQuietly(target.snapshot && target.snapshot.sessionId);
      this.multiConversationItems = this.multiConversationItems.filter((item) => item.id !== conversationId);
      const deletedActive = this.activeConversationId === conversationId;
      if (!this.multiConversationItems.length) this.multiConversationItems.push(this.buildEmptyConversationRecord());
      if (deletedActive) {
        this.multiConversationItems.sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
        const next = this.multiConversationItems[0];
        this.activeConversationId = next.id;
        this.applyConversationSnapshot(next.snapshot || this.buildEmptyConversationRecord().snapshot);
        await this.ensureSession();
      }
      this.persistMultiConversationState();
      this.renderConversationList();
      this.setStatus('已刪除所選對話。', { tone: 'success' });
    }

    async clearAllConversations() {
      if (!this.isMultiConversationEnabled() || this.isBusy) return;
      const confirmed = await this.showActionConfirmation(
        '清除所有對話？',
        '此瀏覽器中的全部前端對話都會移除，且無法復原。後端紀錄仍依站台保存政策保留。',
        '全部清除',
        true
      );
      if (!confirmed) return;
      const sessions = this.multiConversationItems.map((item) => item && item.snapshot && item.snapshot.sessionId).filter(Boolean);
      await Promise.all(sessions.map((sessionId) => this.closeSessionQuietly(sessionId)));
      const next = this.buildEmptyConversationRecord();
      this.multiConversationItems = [next];
      this.activeConversationId = next.id;
      this.resetCurrentConversationState('reset');
      await this.ensureSession();
      this.captureCurrentMultiConversation();
      this.persistMultiConversationState();
      this.closeConversationDrawer();
      this.setStatus('已清除全部前端對話。', { tone: 'success' });
    }
    ...
    formatConversationTime(timestamp) {
      ...
      try { return new Date(value).toLocaleDateString(this.config.languageCode || 'zh-TW'); } catch (_) { return ''; }
    }

    buildConversationDrawer() {
      const overlay = createEl('div', 'ai-assistant-widget-conversation-drawer');
      overlay.hidden = true;
      overlay.setAttribute('aria-hidden', 'true');
      const backdrop = createEl('button', 'ai-assistant-widget-conversation-drawer-backdrop', '');
      backdrop.type = 'button';
      backdrop.setAttribute('aria-label', '關閉對話清單');
      backdrop.addEventListener('click', () => this.closeConversationDrawer());
      const drawer = createEl('section', 'ai-assistant-widget-conversation-drawer-panel');
      drawer.setAttribute('role', 'dialog');
      drawer.setAttribute('aria-modal', 'true');
      drawer.setAttribute('aria-label', '對話清單');
      const head = createEl('div', 'ai-assistant-widget-conversation-drawer-head');
      const copy = createEl('div', 'ai-assistant-widget-conversation-drawer-copy');
      copy.appendChild(createEl('strong', '', '對話清單'));
      copy.appendChild(createEl('span', '', '同一瀏覽器中的不同主題對話'));
      const actions = createEl('div', 'ai-assistant-widget-conversation-drawer-actions');
      const addBtn = createEl('button', 'ai-assistant-widget-conversation-drawer-action is-primary', '');
      addBtn.type = 'button';
      setSvgButtonIcon(addBtn, INLINE_ICON_PATHS.plus, '新增對話');
      addBtn.addEventListener('click', () => this.createNewConversation(true));
      const closeBtn = createEl('button', 'ai-assistant-widget-conversation-drawer-action', '');
      closeBtn.type = 'button';
      setSvgButtonIcon(closeBtn, INLINE_ICON_PATHS.close, '關閉對話清單');
      closeBtn.addEventListener('click', () => this.closeConversationDrawer());
      actions.appendChild(addBtn);
      actions.appendChild(closeBtn);
      head.appendChild(copy);
      head.appendChild(actions);
      const list = createEl('div', 'ai-assistant-widget-conversation-list');
      list.setAttribute('role', 'list');
      this.conversationListEl = list;
      const footer = createEl('div', 'ai-assistant-widget-conversation-drawer-footer');
      const clearAllBtn = createEl('button', 'ai-assistant-widget-conversation-clear-all', '');
      clearAllBtn.type = 'button';
      clearAllBtn.innerHTML = '<svg class="ai-assistant-widget-btn-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">'
        + '<path d="' + INLINE_ICON_PATHS.trash + '" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>'
        + '</svg><span>清除所有對話</span>';
      clearAllBtn.title = '清除所有對話';
      clearAllBtn.setAttribute('aria-label', '清除所有對話');
      clearAllBtn.addEventListener('click', () => this.clearAllConversations());
      footer.appendChild(clearAllBtn);
      footer.appendChild(createEl('span', 'ai-assistant-widget-conversation-drawer-note', '僅清除此瀏覽器的前端紀錄'));
      drawer.appendChild(head);
      drawer.appendChild(list);
      drawer.appendChild(footer);
      overlay.appendChild(backdrop);
      overlay.appendChild(drawer);
      this.conversationDrawerEl = overlay;
      this.renderConversationList();
      return overlay;
    }
    ...
    renderConversationList() {
      ...
      const items = [...this.multiConversationItems].sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
      items.forEach((item) => {
        const row = createEl('div', 'ai-assistant-widget-conversation-item');
        row.setAttribute('role', 'listitem');
        if (item.id === this.activeConversationId) row.classList.add('is-active');
        const selectBtn = createEl('button', 'ai-assistant-widget-conversation-select', '');
        selectBtn.type = 'button';
        selectBtn.setAttribute('aria-current', item.id === this.activeConversationId ? 'true' : 'false');
        selectBtn.appendChild(createEl('strong', 'ai-assistant-widget-conversation-title', item.title || '新對話'));
        const meta = createEl('span', 'ai-assistant-widget-conversation-meta');
        meta.appendChild(createEl('span', '', item.summary || '尚未開始提問'));
        meta.appendChild(createEl('time', '', this.formatConversationTime(item.updatedAt || item.createdAt)));
        selectBtn.appendChild(meta);
        selectBtn.addEventListener('click', () => this.switchConversation(item.id));
        const itemActions = createEl('div', 'ai-assistant-widget-conversation-item-actions');
        const renameBtn = createEl('button', 'ai-assistant-widget-conversation-rename', '');
        renameBtn.type = 'button';
        setSvgButtonIcon(renameBtn, INLINE_ICON_PATHS.edit, `重新命名「${item.title || '新對話'}」`);
        renameBtn.addEventListener('click', (event) => {
          event.stopPropagation();
          this.renameConversation(item.id);
        });
        const deleteBtn = createEl('button', 'ai-assistant-widget-conversation-delete', '');
        deleteBtn.type = 'button';
        setSvgButtonIcon(deleteBtn, INLINE_ICON_PATHS.trash, `刪除「${item.title || '新對話'}」`);
    ...
    openConversationDrawer() {
      if (!this.conversationDrawerEl || this.isBusy) {
        if (this.isBusy) this.setStatus('目前正在處理回答，完成後即可開啟對話清單。', { tone: 'info', duration: 3600 });
        return;
      }
      this.captureCurrentMultiConversation();
      this.renderConversationList();
      this.conversationDrawerLastFocusedEl = document.activeElement;
      this.conversationDrawerEl.hidden = false;
      this.conversationDrawerEl.setAttribute('aria-hidden', 'false');
      this.conversationDrawerEl.classList.add('is-open');
      if (this.rootEl) this.rootEl.classList.add('is-conversation-drawer-open');
      if (this.conversationDrawerBtnEl) this.conversationDrawerBtnEl.setAttribute('aria-expanded', 'true');
      window.setTimeout(() => {
        const active = this.conversationDrawerEl && this.conversationDrawerEl.querySelector('.ai-assistant-widget-conversation-item.is-active .ai-assistant-widget-conversation-select');
        const first = active || (this.conversationDrawerEl && this.conversationDrawerEl.querySelector('button'));
        if (first && typeof first.focus === 'function') first.focus();
      }, 0);
    }

    closeConversationDrawer() {
      if (!this.conversationDrawerEl || this.conversationDrawerEl.hidden) return;
      this.conversationDrawerEl.classList.remove('is-open');
      this.conversationDrawerEl.hidden = true;
      this.conversationDrawerEl.setAttribute('aria-hidden', 'true');
      if (this.rootEl) this.rootEl.classList.remove('is-conversation-drawer-open');
      if (this.conversationDrawerBtnEl) this.conversationDrawerBtnEl.setAttribute('aria-expanded', 'false');
      const focusTarget = this.conversationDrawerLastFocusedEl;
      this.conversationDrawerLastFocusedEl = null;
      if (focusTarget && typeof focusTarget.focus === 'function' && this.rootEl && this.rootEl.classList.contains('open')) {
    ...
    buildAssistantPersistenceRecord(assistantMessageId, text, references, modelName, finishReason, answerMode, responsePolicyCode, responsePolicyName, responseMode, responseModeMatchedBy, responseModeReason, usedOpenAi, usedRetrieval, retrievalMode, referenceCount, groundingLevel, groundingLabel, groundingHint, citationSummary, guardReason, usedN8n, n8nWorkflowCode, n8nRoute, n8nTraceId, n8nResponseType, n8nResponseMode, n8nContractVersion, n8nLatencyMs, fallbackApplied, warnings, openAiCompletionStatus, openAiWasIncomplete, openAiContinuationUsed, openAiContinuationCount, requestedMaxOutputTokens, knowledgeRetrievalDiagnosis, runtimePromptTrace, n8nResponseData, n8nResponseMeta, guidedFaqOptions, answerSourceType, answerSourceQuestion, answerCandidateRequired, answerCandidates, guidedFaqTrail) {
      return {
        ...
        guidedFaqTrail: this.sanitizeGuidedFaqTrail(guidedFaqTrail),
    ...
    showActionConfirmation(title, description, confirmLabel = '確定', danger = false) {
      const fallbackMessage = `${String(title || '確認操作')}\n${String(description || '')}`.trim();
      if (!this.resetConfirmEl) {
        return Promise.resolve(global.confirm ? global.confirm(fallbackMessage) : false);
      }
      if (this.resetConfirmResolve) return Promise.resolve(false);
      this.resetConfirmMode = 'confirm';
      if (this.resetConfirmInputEl) { this.resetConfirmInputEl.hidden = true; this.resetConfirmInputEl.value = ''; }
      if (this.resetConfirmTitleEl) this.resetConfirmTitleEl.textContent = String(title || '確認操作');
      if (this.resetConfirmDescEl) this.resetConfirmDescEl.textContent = String(description || '');
      if (this.resetConfirmOkBtnEl) {
        this.resetConfirmOkBtnEl.textContent = String(confirmLabel || '確定');
        this.resetConfirmOkBtnEl.classList.toggle('is-danger', !!danger);
      }
    ...
    showConversationRenameDialog(currentTitle) {
      if (!this.resetConfirmEl) {
        const value = global.prompt ? global.prompt('重新命名對話', String(currentTitle || '新對話')) : null;
        return Promise.resolve(value == null ? null : String(value).replace(/\s+/g, ' ').trim().slice(0, 40));
      }
      if (this.resetConfirmResolve) return Promise.resolve(null);
      this.resetConfirmMode = 'rename';
      if (this.resetConfirmTitleEl) this.resetConfirmTitleEl.textContent = '重新命名對話';
      if (this.resetConfirmDescEl) this.resetConfirmDescEl.textContent = '輸入容易辨識的對話名稱，最多 40 個字。';
      if (this.resetConfirmInputEl) {
        this.resetConfirmInputEl.hidden = false;
        this.resetConfirmInputEl.value = String(currentTitle || '新對話').slice(0, 40);
      }
      if (this.resetConfirmOkBtnEl) {
        this.resetConfirmOkBtnEl.textContent = '儲存';
        this.resetConfirmOkBtnEl.classList.remove('is-danger');
      }
      this.resetConfirmLastFocusedEl = document.activeElement;
      this.resetConfirmEl.hidden = false;
      this.resetConfirmEl.classList.add('is-visible');
      ...
      return new Promise((resolve) => {
        this.resetConfirmResolve = resolve;
        window.setTimeout(() => {
          if (this.resetConfirmInputEl) { this.resetConfirmInputEl.focus(); this.resetConfirmInputEl.select(); }
        }, 0);
      });
    }

    showResetConfirmation() {
      const description = this.isMultiConversationEnabled()
        ? '重新開始後，目前所選對話的內容會清空並建立新的工作階段；其他對話不受影響。'
        : '重新開始後，畫面上的目前對話會清空，並建立新的工作階段。';
    ...
    closeResetConfirmation(confirmed) {
      const resolvedMode = this.resetConfirmMode;
      ...
      if (this.resetConfirmOkBtnEl) {
        this.resetConfirmOkBtnEl.classList.remove('is-danger');
        this.resetConfirmOkBtnEl.textContent = '確定';
      }
      if (this.resetConfirmInputEl) { this.resetConfirmInputEl.hidden = true; this.resetConfirmInputEl.value = ''; }
      this.resetConfirmMode = 'confirm';
      ...
      if (resolver) resolver(resolvedMode === 'rename' ? (typeof confirmed === 'string' ? confirmed : null) : !!confirmed);
    }
    ...
    async performResetConversation() {
      ...
      if (this.isMultiConversationEnabled()) {
        // 多對話模式的「重新開始」只重設目前所選對話，不新增清單項目，也不影響其他主題。
        let current = this.multiConversationItems.find((item) => item.id === this.activeConversationId);
        if (!current) {
          current = this.buildEmptyConversationRecord();
          this.activeConversationId = current.id;
          this.multiConversationItems.unshift(current);
        }
        const replacement = this.buildEmptyConversationRecord();
        const preservedManualTitle = current.customTitle ? String(current.title || '').trim() : '';
        current.title = preservedManualTitle || replacement.title;
        current.customTitle = !!preservedManualTitle;
        current.summary = replacement.summary;
        current.createdAt = replacement.createdAt;
        current.updatedAt = replacement.updatedAt;
        current.expiresAt = replacement.expiresAt;
        current.snapshot = replacement.snapshot;

        await this.closeSessionQuietly(previousSessionId);
        this.resetCurrentConversationState('reset');
        await this.ensureSession();
        this.captureCurrentMultiConversation();
        this.persistMultiConversationState();
        this.closeConversationDrawer();
        this.setStatus('已重新開始目前對話。', { tone: 'success' });
        return;
      }
    ...
    async sendCurrentMessage() {
      ...
      try {
        ...
        const sendRequest = () => apiFetch(this.config.apiBase, this.config.sendEndpoint, {
          method: 'POST',
        ...
        try {
          ...
          if (!isRecoverableWidgetSessionError(error)) throw error;
          // 前端仍保留歷史訊息，但後端 Session 可能已逾期、關閉或被維運清理；建立新 Session 後重送一次。
          this.sessionId = null;
          await this.ensureSession();
          data = await sendRequest();
          this.setStatus('原工作階段已失效，已建立新的工作階段並繼續提問。', { tone: 'info', duration: 5200 });
        }
    ...
    sanitizeGuidedFaqTrail(items) {
      if (!Array.isArray(items)) return [];
      const result = [];
      items.forEach((item) => {
        const text = trimPersistenceText(String(item || '').trim().replace(/\s+/g, ' '), 80);
        if (!text) return;
        if (result.length === 0 || result[result.length - 1] !== text) result.push(text);
      });
      return result.slice(-5);
    }

    appendGuidedFaqTrailChoice(value) {
      const text = trimPersistenceText(String(value || '').trim().replace(/\s+/g, ' '), 80);
      if (!text) return;
      const current = this.sanitizeGuidedFaqTrail(this.activeGuidedFaqTrail);
      if (current[current.length - 1] !== text) current.push(text);
      this.activeGuidedFaqTrail = this.sanitizeGuidedFaqTrail(current);
    }
    ...
    getGuidedFaqOptionPanels(scope) {
      const root = scope || this.messagesEl || this.rootEl;
      if (!root || typeof root.querySelectorAll !== 'function') return [];
      return Array.from(root.querySelectorAll('.ai-assistant-widget-guided-options'));
    }

    disableGuidedFaqControlsExceptLatest(scope, reasonText = '已有較新的引導選項，舊按鈕已失效。') {
      const panels = this.getGuidedFaqOptionPanels(scope);
      const latestPanel = panels.length > 0 ? panels[panels.length - 1] : null;
      panels.forEach((panel) => {
        if (panel !== latestPanel) this.disableGuidedFaqControls(panel, reasonText);
      });
      return latestPanel;
    }

    enableGuidedFaqControls(scope, titleText = '') {
      const root = scope || this.messagesEl || this.rootEl;
      if (!root || typeof root.querySelectorAll !== 'function') return;
      root.querySelectorAll('.ai-assistant-widget-guided-option, .ai-assistant-widget-guided-exit').forEach((button) => {
        ...
        if (titleText && !button.classList.contains('ai-assistant-widget-guided-exit')) {
          button.title = titleText;
        }
      });
    }

    markLatestGuidedFaqOptionsReusable() {
      const latestPanel = this.disableGuidedFaqControlsExceptLatest(this.messagesEl, '已有較新的引導選項，舊按鈕已失效。');
      if (!latestPanel) return;
      latestPanel.classList.add('is-reselectable');
      const title = latestPanel.querySelector('.ai-assistant-widget-guided-options-title');
      if (title) title.textContent = '可重新選擇下方選項查看不同答案';
      this.enableGuidedFaqControls(latestPanel, '可再次選擇這一項查看不同答案。');
    }

    clearGuidedFaqPanelSelection(panel) {
      if (!panel || typeof panel.querySelectorAll !== 'function') return;
      panel.querySelectorAll('.ai-assistant-widget-guided-option').forEach((button) => {
        button.classList.remove('is-selected');
        button.setAttribute('aria-pressed', 'false');
        if (button.title === '可再次選擇這一項查看不同答案。') return;
        button.removeAttribute('aria-label');
      });
    }

    isGuidedFaqFlowActive() {
      return Array.isArray(this.activeGuidedFaqOptions) && this.activeGuidedFaqOptions.length > 0;
    }

    syncGuidedFaqStatusBar() {
      const isGuidedActive = this.isGuidedFaqFlowActive();
      if (this.guidedFaqStatusBarEl) {
        this.guidedFaqStatusBarEl.hidden = !isGuidedActive;
        this.guidedFaqStatusBarEl.classList.toggle('is-visible', isGuidedActive);
        this.guidedFaqStatusBarEl.setAttribute('aria-hidden', isGuidedActive ? 'false' : 'true');
      }
      if (this.guidedFaqStatusExitBtnEl) {
        const shouldDisable = !isGuidedActive || !!this.isBusy;
        this.guidedFaqStatusExitBtnEl.disabled = shouldDisable;
        this.guidedFaqStatusExitBtnEl.classList.toggle('is-disabled', shouldDisable);
        this.guidedFaqStatusExitBtnEl.setAttribute('aria-disabled', shouldDisable ? 'true' : 'false');
      }
    }
    ...
    exitGuidedFaqFlow() {
      ...
      if (this.guidedFaqStatusExitBtnEl) {
        this.guidedFaqStatusExitBtnEl.disabled = true;
        this.guidedFaqStatusExitBtnEl.classList.add('is-disabled');
        this.guidedFaqStatusExitBtnEl.setAttribute('aria-disabled', 'true');
      }
    ...
    renderAnswerSourceQuestion(sourceType, sourceQuestion, guidedFaqTrail = []) {
      ...
      const panel = createEl('div', normalizedType === 'GUIDED_FAQ' ? 'ai-assistant-widget-answer-source-question is-guided-faq' : 'ai-assistant-widget-answer-source-question');
      const safeTrail = normalizedType === 'GUIDED_FAQ' ? this.sanitizeGuidedFaqTrail(guidedFaqTrail) : [];
      const trailText = safeTrail.length > 0 ? safeTrail.join(' → ') : '';
      panel.setAttribute('aria-label', (normalizedType === 'GUIDED_FAQ' ? '入口問題：' : '來源問題：') + question + (trailText ? '；已選擇：' + trailText : ''));
      const questionRow = createEl('div', 'ai-assistant-widget-answer-source-question-row');
      if (normalizedType === 'GUIDED_FAQ') {
        questionRow.appendChild(createEl('span', 'ai-assistant-widget-answer-source-label', '入口問題'));
      }
      questionRow.appendChild(createEl('span', 'ai-assistant-widget-answer-source-question-text', question));
      panel.appendChild(questionRow);
      if (trailText) {
        const trailRow = createEl('div', 'ai-assistant-widget-answer-source-trail');
        trailRow.title = '已選擇：' + trailText;
        trailRow.appendChild(createEl('span', 'ai-assistant-widget-answer-source-trail-label', '已選擇'));
        trailRow.appendChild(createEl('span', 'ai-assistant-widget-answer-source-trail-text', trailText));
    ...
    renderAnswerCandidates(candidates, required) {
      ...
      items.sort((a, b) => Number(b.score || 0) - Number(a.score || 0)).forEach((item) => {
        ...
        btn.addEventListener('click', () => {
          ...
          this.pendingGuidedFaqSelectedText = '';
          this.pendingGuidedFaqTrailBase = null;
          if (sourceType === 'GUIDED_FAQ') this.activeGuidedFaqTrail = [];
    ...
    renderGuidedFaqOptions(options, showRequiredHint = false, guidedFaqTrailBase = []) {
      ...
      const sanitizedTrailBase = this.sanitizeGuidedFaqTrail(guidedFaqTrailBase);
      panel.dataset.guidedFaqTrailBase = JSON.stringify(sanitizedTrailBase);
      ...
      items.sort((a, b) => Number(a.displayOrder || 0) - Number(b.displayOrder || 0)).forEach((item) => {
        ...
        btn.addEventListener('click', () => {
          ...
          this.clearGuidedFaqPanelSelection(panel);
          this.markGuidedFaqControlSelected(btn, '已選擇');
          // v28.6.19.19.301：不在點擊當下永久 disabled。
          // 送出期間由 isBusy 擋重複送出；若後端回傳下一層選項，舊選項才會失效；若回到最終答案，最後一層仍可重選看不同答案。
          this.pendingGuidedFaqAction = 'SelectOption';
          this.pendingGuidedFaqSelectedText = item.optionText;
          this.pendingGuidedFaqTrailBase = sanitizedTrailBase.slice();
    ...
    appendAssistantMessage(assistantMessageId, text, references, modelName, finishReason, answerMode, responsePolicyCode, responsePolicyName, responseMode, responseModeMatchedBy, responseModeReason, usedOpenAi, usedRetrieval, retrievalMode, referenceCount, groundingLevel, groundingLabel, groundingHint, citationSummary, guardReason, usedN8n, n8nWorkflowCode, n8nRoute, n8nTraceId, n8nResponseType, n8nResponseMode, n8nContractVersion, n8nLatencyMs, fallbackApplied, warnings, openAiCompletionStatus, openAiWasIncomplete, openAiContinuationUsed, openAiContinuationCount, requestedMaxOutputTokens, knowledgeRetrievalDiagnosis, runtimePromptTrace, n8nResponseData, n8nResponseMeta, guidedFaqOptions, answerSourceType, answerSourceQuestion, answerCandidateRequired, answerCandidates, guidedFaqTrailSnapshot) {
      ...
      const isGuidedFaqResponse = String(answerSourceType || '').toUpperCase() === 'GUIDED_FAQ' || String(responseMode || '').toUpperCase() === 'GUIDED_FAQ' || String(answerMode || '').toUpperCase() === 'GUIDED_FAQ';
      const hasGuidedFaqOptions = Array.isArray(guidedFaqOptions) && guidedFaqOptions.length > 0;
      if (!this.isRestoringConversation && isGuidedFaqResponse && !hadActiveGuidedFaqOptionsBefore && this.pendingGuidedFaqAction !== 'SelectOption') {
        this.activeGuidedFaqTrail = [];
      }
      if (!this.isRestoringConversation && isGuidedFaqResponse && this.pendingGuidedFaqAction === 'SelectOption') {
        if (Array.isArray(this.pendingGuidedFaqTrailBase)) {
          this.activeGuidedFaqTrail = this.sanitizeGuidedFaqTrail(this.pendingGuidedFaqTrailBase);
        }
        this.appendGuidedFaqTrailChoice(this.pendingGuidedFaqSelectedText || this.lastUserMessageText);
      }
      const guidedFaqTrailForMessage = Array.isArray(guidedFaqTrailSnapshot)
        ? this.sanitizeGuidedFaqTrail(guidedFaqTrailSnapshot)
        : (isGuidedFaqResponse ? this.sanitizeGuidedFaqTrail(this.activeGuidedFaqTrail) : []);
      this.clearEmptyState();
      ...
      answerTopMeta.appendChild(createEl('div', 'ai-assistant-widget-answer-kicker', isGuidedFaqResponse && hasGuidedFaqOptions ? '請選擇' : '回答'));
      ...
      const sourceQuestionPanel = this.renderAnswerSourceQuestion(answerSourceType, answerSourceQuestion, guidedFaqTrailForMessage);
      ...
      const guidedOptionsPanel = this.renderGuidedFaqOptions(guidedFaqOptions, shouldShowGuidedRequiredHint, guidedFaqTrailForMessage);
      ...
      const shouldKeepLatestGuidedOptionsForReselect = isGuidedFaqResponse
        && nextGuidedFaqOptions.length === 0
        && hadActiveGuidedFaqOptionsBefore
        && this.pendingGuidedFaqAction === 'SelectOption';
      ...
      if (shouldKeepLatestGuidedOptionsForReselect) {
        this.markLatestGuidedFaqOptionsReusable();
      }
      this.activeGuidedFaqOptions = shouldKeepLatestGuidedOptionsForReselect ? this.activeGuidedFaqOptions : nextGuidedFaqOptions;
      if (nextGuidedFaqOptions.length === 0 && !shouldKeepLatestGuidedOptionsForReselect && (hadActiveGuidedFaqOptionsBefore || this.pendingGuidedFaqAction === 'Exit')) {
    ...
    updateBusyState(isBusy) {
      ...
      if (this.headerResetBtnEl) {
        this.headerResetBtnEl.disabled = !!isBusy;
        this.headerResetBtnEl.classList.toggle('is-disabled', !!isBusy);
        this.headerResetBtnEl.setAttribute('aria-disabled', isBusy ? 'true' : 'false');
      }
      ...
      if (this.conversationDrawerBtnEl) {
        this.conversationDrawerBtnEl.disabled = !!isBusy;
        this.conversationDrawerBtnEl.classList.toggle('is-disabled', !!isBusy);
        this.conversationDrawerBtnEl.setAttribute('aria-disabled', isBusy ? 'true' : 'false');
      }
      if (this.conversationDrawerEl) {
        this.conversationDrawerEl.querySelectorAll('button').forEach((btn) => {
          btn.disabled = !!isBusy;
    ...
    handleRootKeydown(e) {
      ...
      if (e.key === 'Escape') {
        e.preventDefault();
        if (this.conversationDrawerEl && !this.conversationDrawerEl.hidden) {
          this.closeConversationDrawer();
          return;
        }
        if (this.resetConfirmEl && !this.resetConfirmEl.hidden) {
          this.closeResetConfirmation(false);
          return;
        }
        this.close();
  ...
  function parseScriptConfig(script) {
    ...
    return {
      ...
      conversationPersistenceEnabled: parseOptionalBoolean(ds.conversationPersistenceEnabled, DEFAULTS.conversationPersistenceEnabled),
      hasCustomConversationPersistenceEnabled: hasDatasetBoolean('conversationPersistenceEnabled'),
      conversationPersistenceHours: ds.conversationPersistenceHours || DEFAULTS.conversationPersistenceHours,
      hasCustomConversationPersistenceHours: hasDatasetText('conversationPersistenceHours'),
      conversationPersistenceMaxMessages: ds.conversationPersistenceMaxMessages || DEFAULTS.conversationPersistenceMaxMessages,
      hasCustomConversationPersistenceMaxMessages: hasDatasetText('conversationPersistenceMaxMessages'),
      multiConversationEnabled: parseOptionalBoolean(ds.multiConversationEnabled, DEFAULTS.multiConversationEnabled),
      hasCustomMultiConversationEnabled: hasDatasetBoolean('multiConversationEnabled'),
      conversationHistoryMaxItems: ds.conversationHistoryMaxItems || DEFAULTS.conversationHistoryMaxItems,
      hasCustomConversationHistoryMaxItems: hasDatasetText('conversationHistoryMaxItems'),
